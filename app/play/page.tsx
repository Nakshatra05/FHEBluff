...
function GameTable({id,address,close,transact}:{id:bigint,address?:`0x${string}`,close:()=>void,transact:(name:'joinTable'|'leaveTable'|'startHand'|'act'|'forceTimeoutFold'|'abortStalledHand',args:readonly unknown[])=>void}){
  ...
  return <div className="fixed inset-0 z-50 overflow-y-auto bg-[#191917] text-white"><header className="sticky top-0 z-20 flex items-center justify-between border-b-3 border-white/25 bg-ink px-3 py-3 sm:px-6"><button onClick={close} className="flex items-center gap-2 font-black"><ArrowLeft/> LOBBY</button><div className="text-center"><p className="font-mono text-[10px] text-acid">TABLE #{id.toString()}</p><p className="font-black">{stage}</p></div><div className="border-2 border-acid px-3 py-2 font-mono text-xs">POT {number(table?.[7])}</div></header><div className="mx-auto grid min-h-[calc(100vh-68px)] max-w-7xl grid-rows-[auto_1fr_auto] gap-4 p-3 pb-28 sm:p-6">
    ...
    <div className="relative grid min-h-[480px] place-items-center overflow-hidden border-3 border-acid bg-[#0f714c] p-3 shadow-[8px_8px_0_#d8ff3e] sm:rounded-[45%]">
      ...
      <div className="relative z-10 text-center"><p className="font-mono text-xs font-bold text-acid">{stage} · {isMyTurn ? 'YOUR TURN' : table ? `SEAT ${number(table[9])+1} TO ACT` : 'SYNCING'}</p><div className="mt-5 flex justify-center gap-2">{Array.from({length:5},(_,i)=><PlayingCard key={i} value={community?.[i]}/>)}</div><div className="mt-6 inline-block border-3 border-ink bg-acid px-5 py-3 font-heading text-2xl text-ink">{number(table?.[7])} CHIPS</div></div>
    ...
    <div className="sticky bottom-16 z-20 border-3 border-ink bg-cream p-3 text-ink shadow-hard sm:bottom-3">
      ...
      {me<0 ? <button onClick={()=>transact('joinTable',[id,BigInt(table?.[3]||1000)])} className="brutal-button w-full bg-acid py-4">JOIN FOR {number(table?.[3])} CHIPS</button>
      ...
      : <><div className="mb-2 grid grid-cols-2 gap-2"><button disabled={privateBusy} onClick={decryptMine} className="min-h-12 border-2 border-ink bg-white px-3 py-2 text-xs font-black disabled:cursor-wait disabled:opacity-50"><Eye/> {privateBusy?'WORKING…':'VIEW MY CARDS'}</button>{timedOut?<button onClick={()=>transact('forceTimeoutFold',[id])} className="min-h-12 border-2 border-ink bg-pink px-3 py-2 text-xs font-black">SKIP TIMED-OUT PLAYER</button>:<div className="grid place-items-center border-2 border-ink bg-white px-2 text-center font-mono text-[10px] font-bold">{isMyTurn?'YOUR MOVE':'WAITING FOR PLAYER'}</div>}</div>{isMyTurn?<div className="grid grid-cols-2 gap-2 sm:grid-cols-5"><Action label="FOLD" color="bg-pink" onClick={()=>transact('act',[id,ACTIONS.FOLD,0n])}/><Action label={number(table?.[8])===number(seats?.[2][me])?'CHECK':`CALL ${Math.max(0,number(table?.[8])-number(seats?.[2][me]))}`} color="bg-green" onClick={()=>transact('act',[id,number(table?.[8])===number(seats?.[2][me])?ACTIONS.CHECK:ACTIONS.CALL,0n])}/><input aria-label="Raise to" type="number" value={raise} onChange={e=>setRaise(Number(e.target.value))} className="input-brutal min-w-0"/><Action label="RAISE" color="bg-acid" onClick={()=>transact('act',[id,ACTIONS.RAISE,BigInt(raise)])}/><Action label="ALL-IN" color="bg-purple text-white" onClick={()=>transact('act',[id,ACTIONS.ALL_IN,0n])}/></div>:<div className="border-3 border-ink bg-white p-3 text-center text-sm font-black">ACTION CONTROLS APPEAR ON YOUR TURN</div>}</>}
    </div>
  </div></div>
}
