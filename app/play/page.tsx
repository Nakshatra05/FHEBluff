...
function GameTable({id,address,close,transact}:{id:bigint,address?:`0x${string}`,close:()=>void,transact:(name:'joinTable'|'leaveTable'|'startHand'|'act'|'forceTimeoutFold'|'abortStalledHand',args:readonly unknown[])=>void}){
  ...
  const isMyTurn=me>=0&&number(table?.[9])===me;
  ...
  const advanceEncryptedShuffle=async()=>{try{setPrivacyStatus('Preparing the next private deal batch…');await freshPrivateWrite({address:POKER_ADDRESS,abi:fheBluffAbi,functionName:'advanceShuffle',args:[id,4]});setPrivacyStatus('Deal batch ready. Confirm in your wallet.');}catch(e){setPrivacyStatus(e instanceof Error?e.message:'Could not prepare the deal transaction');}};
