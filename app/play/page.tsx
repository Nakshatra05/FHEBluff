...
function handInsight(cards:number[]){
  if(cards.length<2)return 'Decrypt your cards to unlock a private read.';
  if(cards.length===2){const [a,b]=cards;const high=(a%13)>(b%13)?a:b;const pair=a%13===b%13;const suited=Math.floor(a/13)===Math.floor(b/13);const connected=Math.abs(a%13-b%13)===1;return pair?`POCKET ${rankName(a)}s · MADE HAND`:suited&&connected?`${rankName(high)}-HIGH SUITED CONNECTOR · DRAW POWER`:suited?`${rankName(high)}-HIGH SUITED · FLUSH POTENTIAL`:connected?`${rankName(high)}-HIGH CONNECTOR · STRAIGHT POTENTIAL`:`${rankName(high)}-HIGH · PICK YOUR SPOT`;}
  const ranks=cards.map(card=>card%13+2);const suits=cards.map(card=>Math.floor(card/13));const rankCounts=new Map<number,number>();const suitCounts=new Map<number,number>();ranks.forEach(r=>rankCounts.set(r,(rankCounts.get(r)||0)+1));suits.forEach(s=>suitCounts.set(s,(suitCounts.get(s)||0)+1));
  const straightHigh=(values:number[])=>{const unique=[...new Set(values.includes(14)?[...values,1]:values)].sort((a,b)=>a-b);let run=1,best=0;for(let i=1;i<unique.length;i++){if(unique[i]===unique[i-1]+1){run++;if(run>=5)best=unique[i];}else run=1;}return best;};
  const flushSuit=[...suitCounts.entries()].find(([,count])=>count>=5)?.[0];const straight=straightHigh(ranks);const straightFlush=flushSuit!==undefined&&straightHigh(cards.filter(card=>Math.floor(card/13)===flushSuit).map(card=>card%13+2));const groups=[...rankCounts.values()].sort((a,b)=>b-a);
  if(straightFlush)return 'STRAIGHT FLUSH · MONSTER';if(groups[0]===4)return 'FOUR OF A KIND · MONSTER';if(groups[0]===3&&groups[1]>=2)return 'FULL HOUSE · LOCKED IN';if(flushSuit!==undefined)return 'FLUSH · VERY STRONG';if(straight)return 'STRAIGHT · VERY STRONG';if(groups[0]===3)return 'THREE OF A KIND · STRONG';if(groups[0]===2&&groups[1]===2)return 'TWO PAIR · SOLID';if(groups[0]===2)return 'ONE PAIR · STAY SHARP';return 'HIGH CARD · BLUFF WINDOW';
}
...
function GameTable({id,address,close,transact}:{id:bigint,address?:`0x${string}`,close:()=>void,transact:(name:'joinTable'|'leaveTable'|'startHand'|'act'|'forceTimeoutFold'|'abortStalledHand',args:readonly unknown[])=>void}){
  ...
  const isMyTurn=phase>=2&&phase<=5&&me>=0&&number(table?.[9])===me;
  ...
  const makeMove=(action:number,amount=0n)=>{playTone(action===ACTIONS.ALL_IN?720:action===ACTIONS.RAISE?620:420);transact('act',[id,action,amount]);};
  ...
  const runPrivateTask=async(task:()=>Promise<void>)=>{if(privateLock.current)return;privateLock.current=true;setPrivateLocked(true);try{await task();}finally{privateLock.current=false;setPrivateLocked(false);}};
  const freshPrivateWrite=async(request:Parameters<typeof writePrivate>[0])=>{if(!publicClient)throw new Error('Network client unavailable');const fees=await publicClient.estimateFeesPerGas({type:'eip1559'});setPrivacyStatus('Check your wallet to continue.');const hash=await writePrivate({...request,maxFeePerGas:fees.maxFeePerGas*2n,maxPriorityFeePerGas:fees.maxPriorityFeePerGas} as never);setPrivacyStatus('Confirming on Arbitrum Sepolia…');await publicClient.waitForTransactionReceipt({hash});await Promise.all([refetchView(),refetchSeats(),refetchCommunity(),refetchShuffle(),refetchEntropy()]);};
  const advanceEncryptedShuffle=()=>void runPrivateTask(async()=>{try{setPrivacyStatus('Preparing the next private deal batch…');await freshPrivateWrite({address:POKER_ADDRESS,abi:fheBluffAbi,functionName:'advanceShuffle',args:[id,4]});setPrivacyStatus('Deal batch confirmed.');}catch(e){setPrivacyStatus(e instanceof Error?e.message:'Could not prepare the deal transaction');}});
  const submitEncryptedEntropy=()=>void runPrivateTask(async()=>{try{setPrivacyStatus('Generating private entropy and ZK proof…');await connectCofhe();const words=new BigUint64Array(2);crypto.getRandomValues(words);const entropy=(words[0]<<64n)|words[1];const [handle,proof]=await cofheClient.encryptInputs([Encryptable.uint128(entropy)]).setConsumingContract(POKER_ADDRESS).execute();await freshPrivateWrite({address:POKER_ADDRESS,abi:fheBluffAbi,functionName:'submitEntropy',args:[id,handle,proof]});setPrivacyStatus('Private entropy confirmed.');}catch(e){setPrivacyStatus(e instanceof Error?e.message:'Encryption failed');}});
  ...
  return <div className="fixed inset-0 z-50 overflow-y-auto bg-[#191917] text-white"><header className="sticky top-0 z-20 flex items-center justify-between border-b-3 border-white/25 bg-ink px-3 py-3 sm:px-6"><button onClick={close} className="flex items-center gap-2 font-black"><ArrowLeft/> LOBBY</button><div className="text-center"><p className="font-mono text-[10px] text-acid">TABLE #{id.toString()}</p><p className="font-black">{stage}</p></div><div className="border-2 border-acid px-3 py-2 font-mono text-xs">POT {number(table?.[7])}</div></header><div className="mx-auto grid min-h-[calc(100vh-68px)] max-w-7xl grid-rows-[auto_1fr_auto] gap-4 p-3 pb-28 sm:p-6">
    ...
    <div className="grid min-h-0 gap-4 lg:grid-cols-[minmax(0,1fr)_260px]">
      <div className="relative grid min-h-[480px] place-items-center overflow-hidden border-3 border-acid bg-[#0f714c] p-3 shadow-[8px_8px_0_#d8ff3e] sm:rounded-[45%]">
        ...
        <div className="absolute inset-x-3 top-3 flex justify-center gap-2 sm:inset-x-24 sm:justify-between">{seats?.[0].slice(0,3).map((p,i)=><Seat key={p} player={p} stack={number(seats[1][i])} bet={number(seats[2][i])} state={seats[3][i]} active={number(table?.[9])===i} credits={seatCredits[i]||0}/>)}</div>
        <div className="absolute inset-x-3 bottom-3 flex justify-center gap-2 sm:inset-x-24 sm:justify-between">{seats?.[0].slice(3,6).map((p,j)=>{const i=j+3;return <Seat key={p} player={p} stack={number(seats[1][i])} bet={number(seats[2][i])} state={seats[3][i]} active={number(table?.[9])===i} credits={seatCredits[i]||0}/>})}</div>
      </div>
    ...
    <div className="sticky bottom-16 z-20 border-3 border-ink bg-cream p-3 text-ink shadow-hard sm:bottom-3">
      ...
      : <button onClick={()=>transact('joinTable',[id,BigInt(table[3])])} className="brutal-button w-full bg-acid py-4">JOIN FOR {number(table[3])} CHIPS</button>
      : phase===0||phase===7 ? <div className="grid grid-cols-2 gap-2"><button onClick={()=>transact('leaveTable',[id])} className="brutal-button bg-white py-4">LEAVE TABLE</button>{address?.toLowerCase()===table?.[0]?.toLowerCase()&&number(table?.[4])>=2?<button onClick={()=>transact('startHand',[id])} className="brutal-button bg-pink py-4">START QUICK HAND</button>:<div className="grid place-items-center border-3 border-ink bg-white p-2 text-center text-xs font-black">WAITING FOR HOST</div>}</div>
      ...
      : phase===6 ? <button disabled={privateBusy} onClick={()=>publishReveal(true)} className="brutal-button w-full bg-pink py-4 disabled:cursor-wait disabled:opacity-50"><ShieldCheck/> {privateBusy?'SETTLING…':'SETTLE HAND'}</button>
      ...
      : <><div className="mb-2 grid grid-cols-2 gap-2"><button disabled={privateBusy} onClick={decryptMine} className="min-h-12 border-2 border-ink bg-white px-3 py-2 text-xs font-black disabled:cursor-wait disabled:opacity-50"><Eye/> {privateBusy?'WORKING…':'VIEW MY CARDS'}</button>{timedOut?<button onClick={()=>transact('forceTimeoutFold',[id])} className="min-h-12 border-2 border-ink bg-pink px-3 py-2 text-xs font-black">SKIP TIMED-OUT PLAYER</button>:<div className="grid place-items-center border-2 border-ink bg-white px-2 text-center font-mono text-[10px] font-bold">{isMyTurn?<span className="flex items-center gap-1 text-purple"><Timer className="size-3"/> YOUR MOVE · {secondsLeft}s</span>:'WAITING FOR PLAYER'}</div>}</div>{isMyTurn?<div className="grid grid-cols-2 gap-2 sm:grid-cols-5"><Action label="FOLD" color="bg-pink" onClick={()=>makeMove(ACTIONS.FOLD)}/><Action label={number(table?.[8])===number(seats?.[2][me])?'CHECK':`CALL ${Math.max(0,number(table?.[8])-number(seats?.[2][me]))}`} color="bg-green" onClick={()=>makeMove(number(table?.[8])===number(seats?.[2][me])?ACTIONS.CHECK:ACTIONS.CALL)}/><input aria-label="Raise to" type="number" min={number(table?.[8])+number(table?.[2])*2} max={me>=0?number(seats?.[1][me])+number(seats?.[2][me]):undefined} value={raise} onChange={e=>setRaise(Number(e.target.value))} className="input-brutal min-w-0"/><Action label="RAISE" color="bg-acid" onClick={()=>makeMove(ACTIONS.RAISE,BigInt(raise))}/><Action label="ALL-IN" color="bg-purple text-white" onClick={()=>makeMove(ACTIONS.ALL_IN)}/></div>:<div className="border-3 border-ink bg-white p-3 text-center text-sm font-black">ACTION CONTROLS APPEAR ON YOUR TURN</div>}</>}
    </div>
  </div></div>
}
