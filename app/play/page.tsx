...
export default function PokerApp() {
  ...
  const transact = (functionName:'createTable'|'joinTable'|'startHand'|'act', args:readonly unknown[]) => {
    if(!authenticated) return login();
    if(wrongNetwork) return switchChain({chainId:ARBITRUM_SEPOLIA_CHAIN_ID});
    writeContract({ address:POKER_ADDRESS, abi:fheBluffAbi, functionName, args } as never);
  };
    ...
    <main className="min-h-screen bg-[#ece7d7] text-ink">
      <header className="sticky top-0 z-40 border-b-3 border-ink bg-cream">
        <div className="mx-auto flex max-w-[1500px] items-center justify-between gap-3 px-3 py-3 sm:px-6">
          <div className="flex items-center gap-3"><Link href="/" aria-label="Back to landing page" className="grid size-10 place-items-center border-3 border-ink bg-white shadow-hard-sm"><ArrowLeft className="size-5" /></Link><div className="flex items-center gap-2 font-black"><Spade className="size-6 fill-current" /> <span className="hidden sm:inline">FHEBLUFF</span></div></div>
          ...
          <div className="flex items-center gap-2">
            {authenticated && <div className="hidden border-3 border-ink bg-acid px-3 py-2 text-sm font-black sm:block">{number(creditData)} CR</div>}
      ...
      {(isPending || isConfirming || isSuccess || error) && <div className={`fixed bottom-4 right-4 z-50 max-w-sm border-3 border-ink p-4 font-bold shadow-hard ${error?'bg-pink':isSuccess?'bg-green':'bg-acid'}`}>{error ? `Transaction failed: ${error.message.split('\n')[0]}` : isSuccess ? 'Transaction confirmed onchain.' : isConfirming ? 'Confirming on Arbitrum Sepolia…' : 'Check your wallet to continue.'}</div>}
      ...
      <div className="mx-auto grid max-w-[1500px] gap-5 p-3 sm:p-6 xl:grid-cols-[230px_1fr]">
        <aside className="hidden self-start border-3 border-ink bg-ink p-4 text-white shadow-hard xl:block">
          ...
          <div className="mt-7 space-y-2">{[['Lobby',Users],['Active hands',Activity],['Credits',Coins],['Leaderboard',Trophy]].map(([label,Icon],i)=><div key={String(label)} className={`flex items-center gap-3 border-2 border-white/30 px-3 py-3 font-bold ${i===0?'bg-acid text-ink':''}`}><Icon className="size-5" />{String(label)}</div>)}</div>
        ...
        <section className="min-w-0">
          <div className="mb-5 flex flex-col justify-between gap-4 sm:flex-row sm:items-end">
            <div><p className="eyebrow text-purple">ONCHAIN LOBBY</p><h1 className="font-heading text-4xl uppercase leading-none sm:text-6xl">Pick your poison.</h1></div>
          ...
          <Tabs defaultValue="tables">
            ...
            <TabsContent value="leaderboard" className="mt-5"><Leaderboard address={address} credits={number(creditData)} /></TabsContent>
      ...
      <nav className="fixed inset-x-0 bottom-0 z-30 grid grid-cols-4 border-t-3 border-ink bg-cream xl:hidden">{[[Users,'Lobby'],[Activity,'Hands'],[Coins,'Credits'],[Trophy,'Ranks']].map(([Icon,label],i)=><button key={String(label)} className={`grid min-h-16 place-items-center text-[11px] font-black ${i===0?'bg-acid':''}`}><Icon className="size-5" />{String(label)}</button>)}</nav>
    </main>
  );
}
...
function Leaderboard({address,credits}:{address?:string,credits:number}){return <div className="border-3 border-ink bg-white shadow-hard"><div className="flex items-center justify-between border-b-3 border-ink bg-acid p-5"><h2 className="font-heading text-3xl">CREDITS BOARD</h2><Crown/></div><div className="p-5"><div className="grid grid-cols-[50px_1fr_auto] items-center gap-3 border-3 border-ink bg-cream p-4"><span className="font-heading text-2xl">—</span><span className="font-mono font-bold">{address?short(address):'CONNECT TO SEE YOUR RANK'}</span><strong>{credits} CR</strong></div><p className="mt-5 text-sm font-semibold text-ink/60">Leaderboard entries are derived from HandSettled events. Every tied winner receives one Credit; Credits cannot be transferred.</p></div></div>}
...
function GameTable({id,address,close,transact}:{id:bigint,address?:`0x${string}`,close:()=>void,transact:(name:'joinTable'|'startHand'|'act',args:readonly unknown[])=>void}){
  const [raise,setRaise]=useState(40);
  const [privateCards,setPrivateCards]=useState<number[]>([]);
  const [privacyStatus,setPrivacyStatus]=useState('');
  const publicClient=usePublicClient(); const {data:walletClient}=useWalletClient(); const {writeContract:writePrivate}=useWriteContract();
  ...
  return <div className="fixed inset-0 z-50 overflow-y-auto bg-[#191917] text-white"><header className="sticky top-0 z-20 flex items-center justify-between border-b-3 border-white/25 bg-ink px-3 py-3 sm:px-6"><button onClick={close} className="flex items-center gap-2 font-black"><ArrowLeft/> LOBBY</button><div className="text-center"><p className="font-mono text-[10px] text-acid">TABLE #{id.toString()}</p><p className="font-black">{stage}</p></div><div className="border-2 border-acid px-3 py-2 font-mono text-xs">POT {number(table?.[7])}</div></header><div className="mx-auto grid min-h-[calc(100vh-68px)] max-w-7xl grid-rows-[auto_1fr_auto] gap-4 p-3 pb-28 sm:p-6">
    ...
    <div className="sticky bottom-16 z-20 border-3 border-ink bg-cream p-3 text-ink shadow-hard sm:bottom-3"><div className="mb-3 flex items-center justify-between"><div className="flex gap-2"><PlayingCard value={privateCards[0]} hidden={privateCards.length===0}/><PlayingCard value={privateCards[1]} hidden={privateCards.length===0}/></div><div className="max-w-[60%] text-right"><p className="font-mono text-[10px] font-bold">PRIVATE HAND</p><p className="text-xs font-semibold text-purple"><LockKeyhole className="inline size-3"/> {privateCards.length?'DECRYPTED LOCALLY':'ACP LOCKED'}</p>{privacyStatus&&<p className="mt-1 truncate text-[10px] font-bold text-ink/55">{privacyStatus}</p>}</div></div>{me<0 ? <button onClick={()=>transact('joinTable',[id,BigInt(table?.[3]||1000)])} className="brutal-button w-full bg-acid py-4">JOIN FOR {number(table?.[3])} CHIPS</button> : number(table?.[5])===1 ? number(shuffleRemaining)>0 ? <div><p className="mb-2 text-center font-mono text-xs font-bold">ENCRYPTED SHUFFLE · {number(shuffleRemaining)}/51 STEPS REMAIN</p><button onClick={()=>writePrivate({address:POKER_ADDRESS,abi:fheBluffAbi,functionName:'advanceShuffle',args:[id,2]})} className="brutal-button w-full bg-acid py-4"><ShieldCheck/> ADVANCE PRIVATE SHUFFLE</button></div> : entropySubmitted ? <div className="border-3 border-ink bg-white p-4 text-center font-black">ENTROPY LOCKED · WAITING FOR OTHER PLAYERS</div> : <button onClick={submitEncryptedEntropy} className="brutal-button w-full bg-purple py-4 text-white"><LockKeyhole/> SUBMIT ENCRYPTED ENTROPY</button> : number(table?.[5])===6 ? <button onClick={()=>publishReveal(true)} className="brutal-button w-full bg-pink py-4"><ShieldCheck/> VERIFY & SETTLE SHOWDOWN</button> : <><div className="mb-2 grid grid-cols-2 gap-2"><button onClick={decryptMine} className="border-2 border-ink bg-white px-3 py-2 text-xs font-black"><Eye/> DECRYPT MY CARDS</button>{number(table?.[5])>=3&&number(table?.[5])<=5&&<button onClick={()=>publishReveal(false)} className="border-2 border-ink bg-green px-3 py-2 text-xs font-black"><ShieldCheck/> PUBLISH STREET</button>}</div><div className="grid grid-cols-3 gap-2 sm:grid-cols-6"><Action label="FOLD" color="bg-pink" onClick={()=>transact('act',[id,ACTIONS.FOLD,0n])}/><Action label="CHECK" onClick={()=>transact('act',[id,ACTIONS.CHECK,0n])}/><Action label="CALL" color="bg-green" onClick={()=>transact('act',[id,ACTIONS.CALL,0n])}/><input aria-label="Raise amount" type="number" value={raise} onChange={e=>setRaise(Number(e.target.value))} className="input-brutal min-w-0"/><Action label="RAISE" color="bg-acid" onClick={()=>transact('act',[id,ACTIONS.RAISE,BigInt(raise)])}/><Action label="ALL-IN" color="bg-purple text-white" onClick={()=>transact('act',[id,ACTIONS.ALL_IN,0n])}/></div></>}</div>
  </div></div>
}
