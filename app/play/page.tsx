...
function PokerApp() {
  const {address:POKER_ADDRESS,version}=usePokerDeployment();
  ...
  useEffect(()=>{
    ...
    const load=async()=>{if(fetching)return;fetching=true;try{
      const histories=await Promise.all((Object.entries(deployments) as [DeploymentVersion,(typeof deployments)[DeploymentVersion]][]).map(async([version,deployment])=>{
...
function GameTable({id,address,close,transact,busy,notify}:{id:bigint,address?:`0x${string}`,close:()=>void,busy:boolean,notify:(notice:TransactionFeedback)=>void,transact:(name:'joinTable'|'leaveTable'|'startHand'|'act'|'forceTimeoutFold'|'abortStalledHand'|'confirmCardsReady',args:readonly unknown[])=>void}){
  const {address:POKER_ADDRESS,block:POKER_DEPLOYMENT_BLOCK,readiness,version}=usePokerDeployment();
  ...
  const inviteFriend=async()=>{const url=new URL('/play',window.location.origin);url.searchParams.set('table',id.toString());url.searchParams.set('version',version);try{await navigator.clipboard.writeText(url.toString());setInviteStatus('Invite copied — send it to a friend');}catch{setInviteStatus(url.toString());}};
