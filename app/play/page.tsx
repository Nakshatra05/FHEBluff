...
import { ReadyToPlay } from '@/components/poker/ready-to-play';
...
export default function PokerPage(){
  ...
  useEffect(()=>{const timer=setTimeout(()=>setVersion(resolveDeployment(window.location.search)),0);return()=>clearTimeout(timer);},[]);
  if(!version)return <main className="min-h-screen bg-cream p-8 font-bold">Opening the poker lobby…</main>;
  return <PokerDeploymentContext.Provider value={deployments[version]}><PokerApp key={version}/></PokerDeploymentContext.Provider>;
}
...
function PokerApp() {
  ...
  const {creditData,leaderData}=useCreditsBoard(address);
  ...
  const transact = async (functionName:'createTable'|'joinTable'|'leaveTable'|'startHand'|'act'|'forceTimeoutFold'|'abortStalledHand'|'confirmCardsReady', args:readonly unknown[]) => {
  ...
  const activeRows = rows.filter(({data})=>data && ((number(data[5])>0 && number(data[5])<7)||number(data[5])===9||(number(data[5])===0&&number(data[4])>=number(data[1]))));
      ...
      <header className="sticky top-0 z-40 border-b-3 border-ink bg-cream">
        <div className="mx-auto flex max-w-[1500px] items-center justify-between gap-3 px-3 py-3 sm:px-6">
          ...
          <div className="flex items-center gap-2">
            ...
            {!ready ? <div className="h-11 w-32 animate-pulse border-3 border-ink bg-white" /> : authenticated ? <WalletMenu key={address||'disconnected'} address={address} profile={()=>setAppView('credits')} logout={()=>void logout()}/> : <button onClick={login} className="brutal-button bg-pink px-4 py-2 text-base">CONNECT</button>}
          </div>
        </div>
      </header>
      ...
      <div className="mx-auto max-w-6xl p-3 sm:p-6">
        <section className="min-w-0">
          <div className="mb-5 flex flex-col justify-between gap-4 sm:flex-row sm:items-end">
            ...
            <Dialog open={createOpen} onOpenChange={setCreateOpen}><DialogTrigger render={<button className="brutal-button bg-acid px-5 py-3"><Plus /> CREATE TABLE</button>} /><DialogContent className="neo-dialog border-3 border-ink bg-cream shadow-hard-lg max-h-[calc(100dvh-2rem)] overflow-y-auto sm:max-w-md"><DialogHeader><DialogTitle className="font-heading text-3xl uppercase">Create a table</DialogTitle><DialogDescription className="font-semibold text-ink/70">Pick a pace, invite rivals, and fight for onchain Credits.</DialogDescription></DialogHeader><div className="space-y-5 pt-3">{notice&&<TransactionNotice notice={notice} onDismiss={dismissNotice} inline/>}<div><p className="mb-2 font-mono text-sm font-bold">QUICK PRESETS</p><div className="grid grid-cols-3 gap-2">{([{label:'DUEL',players:2,blind:5,buyIn:500},{label:'TURBO',players:4,blind:10,buyIn:1000},{label:'CHAOS',players:6,blind:25,buyIn:2500}] as const).map(preset=><button key={preset.label} aria-pressed={maxPlayers===preset.players&&smallBlind===preset.blind&&minBuyIn===preset.buyIn} onClick={()=>{setMaxPlayers(preset.players);setSmallBlind(preset.blind);setMinBuyIn(preset.buyIn);}} className="table-preset border-2 border-ink bg-white p-2 text-sm font-black hover:bg-acid"><Zap className="mx-auto mb-1 size-4"/>{preset.label}<span className="mt-1 block text-xs font-semibold">{preset.players} seats · {preset.buyIn} chips</span></button>)}</div></div><Field label="Seats"><select value={maxPlayers} onChange={e=>setMaxPlayers(Number(e.target.value))} className="input-brutal"><option value="2">Heads-up · 2</option><option value="4">Four-max · 4</option><option value="6">Six-max · 6</option></select></Field><Field label="Small blind"><input className="input-brutal" type="number" min="1" value={smallBlind} onChange={e=>setSmallBlind(Number(e.target.value))}/></Field><Field label="Free starting chips"><input className="input-brutal" type="number" min="20" value={minBuyIn} onChange={e=>setMinBuyIn(Number(e.target.value))}/></Field><button disabled={!contractReady||submissionLocked} onClick={createTable} className="brutal-button w-full bg-purple px-5 py-4 text-white disabled:cursor-not-allowed disabled:opacity-40">{submissionLocked?'TRANSACTION IN PROGRESS':!authenticated?'CONNECT TO CREATE':wrongNetwork?'SWITCH NETWORK':'CREATE ONCHAIN'}</button></div></DialogContent></Dialog>
          </div>
...
function GameTable({id,address,close,transact,busy,notify}:{id:bigint,address?:`0x${string}`,close:()=>void,busy:boolean,notify:(notice:TransactionFeedback)=>void,transact:(name:'joinTable'|'leaveTable'|'startHand'|'act'|'forceTimeoutFold'|'abortStalledHand'|'confirmCardsReady',args:readonly unknown[])=>void}){
  const {address:POKER_ADDRESS,block:POKER_DEPLOYMENT_BLOCK,readiness}=usePokerDeployment();
  const cofheClient=useMemo(()=>createCardViewClient(POKER_ADDRESS),[POKER_ADDRESS]);
  const {data:cardsAcknowledged}=useReadContract({address:POKER_ADDRESS,abi:fheBluffAbi,functionName:'isPlayerReady',args:address?[id,address]:undefined,query:{enabled:readiness&&!!address,refetchInterval:2000}});
  ...
  },[publicClient,id,resultPhase,resultHandId,refetchView,POKER_ADDRESS,POKER_DEPLOYMENT_BLOCK]);
  ...
  const cardView=usePrivateCards(id,table?.[6],address,me>=0&&((phase>=2&&phase<=6)||phase===9));
  ...
  const tableStatus=!table?'SYNCING':phase===9?'READY TO PLAY · CLOCK OFF':phase===8?'TABLE CLOSED · NO ACTIONS':phase===7?'HAND COMPLETE · READY FOR NEXT':phase===0?'SEATING · WAITING FOR PLAYERS':isMyTurn?'YOUR TURN':`SEAT ${number(table[9])+1} TO ACT`;
  ...
  const inviteFriend=async()=>{const url=new URL('/play',window.location.origin);url.searchParams.set('table',id.toString());url.searchParams.set('version',readiness?'ready':'legacy');try{await navigator.clipboard.writeText(url.toString());setInviteStatus('Invite copied — send it to a friend');}catch{setInviteStatus(url.toString());}};
  ...
  return <div className="fixed inset-0 z-50 overflow-y-auto bg-[#191917] text-white"><header className="sticky top-0 z-20 flex items-center justify-between border-b-3 border-white/25 bg-ink px-3 py-3 sm:px-6"><button onClick={close} className="flex items-center gap-2 font-black"><ArrowLeft/> LOBBY</button><div className="text-center"><p className="font-mono text-xs text-acid">{tableLabel(id,readiness?'ready':'legacy')}</p><p className="font-black">{stage}</p></div><div className="border-2 border-acid px-3 py-2 font-mono text-sm">POT {number(table?.[7])}</div></header><div className="poker-layout mx-auto grid max-w-7xl">
    ...
    <div aria-label="Game controls" className="poker-controls border-3 border-ink bg-cream p-3 text-ink shadow-hard">
      ...
      {me>=0&&((phase>=2&&phase<=6)||phase===9)&&<CardPreparation protectedDeal={phase===9} stage={cardView.stage} elapsed={cardView.elapsed} message={cardView.message} busy={cardView.busy} onRetry={()=>void cardView.load()} onStop={cardView.stop} disabled={privateBusy||busy}/>}
