...
export function PlayLaunchpad({practice,quickSeat,hasSeat,loading,address,credits,totals}:{practice:()=>void;quickSeat:()=>void;hasSeat:boolean;loading:boolean;address?:string;credits?:bigint;totals?:readonly bigint[]}) {
  ...
  return <section className="play-launchpad mb-5">
    ...
    <details className="mt-3 border-b-2 border-ink/20"><summary className="flex min-h-11 cursor-pointer items-center gap-2 text-sm font-bold"><BookOpen size={16}/>New here? The 30-second guide</summary><div className="grid gap-4 pb-4 text-sm sm:grid-cols-3"><p><strong className="block">1. Get your cards</strong>Join with free chips. Ready the private deal, then unlock your two cards. Only you can view them.</p><p><strong className="block">2. Make your move</strong>Check costs nothing. Call matches the bet. Raise adds pressure. Fold leaves this hand, not the table.</p><p><strong className="block">3. Win the pot</strong>Make the best five-card hand, or be the last player who has not folded. Verified multiplayer winners earn +1 Credit; practice does not.</p></div></details>
  </section>;
}
