...
import {ShowdownRecap} from './showdown-recap';
import type {PublicHandRecap} from '@/lib/public-hand-recap';

type Result={handId:bigint;winners:readonly string[];pot:bigint;transactionHash:string;method?:string;recap?:PublicHandRecap};
export function HandResultPanel({handId,result,address,seated,folded,onLobby}:{handId:bigint;result:Result|null;address?:string;seated:boolean;folded:boolean;onLobby:()=>void}) {
  ...
  return <section aria-label="Hand result" className={`hand-result border-3 border-ink p-5 text-ink shadow-hard sm:p-7 ${won?'bg-acid':'bg-cream'}`}>
    ...
    {confirmed?.recap&&<ShowdownRecap recap={confirmed.recap} winners={confirmed.winners}/>}
    <div className="mt-5 flex flex-wrap gap-3"><button onClick={onLobby} className="brutal-button min-h-14 w-full bg-pink px-6 py-4 text-lg">BACK TO LOBBY</button>{confirmed&&<a href={`https://sepolia.arbiscan.io/tx/${confirmed.transactionHash}`} target="_blank" rel="noreferrer" className="inline-flex min-h-12 items-center border-2 border-ink bg-white px-4 py-3 font-bold underline">View settlement receipt ↗</a>}</div><p className="mt-3 text-sm font-semibold">This hand is complete. Back to lobby needs no signature; it does not withdraw your seat’s chips.</p>
  </section>;
}
