...
export const deployments={
  ...
  flow:{address:'0xa6B08ad65B2935A0520D4B5A5Dca1EC7B2b775A1' as `0x${string}`,block:306151862n,readiness:false,version:'flow' as const},
};
...
export function resolveDeployment(search:string):DeploymentVersion{
  ...
  if(!query.has('table'))return 'flow';
  return query.get('version')==='flow'?'flow':query.get('version')==='instant'?'instant':query.get('version')==='ready'?'ready':'legacy';
}
export const PokerDeploymentContext=createContext<(typeof deployments)[DeploymentVersion]>(deployments.flow);
