...
contract FHEBluff {
    ...
    function startHand(uint256 tableId) external {
        ...
        if ((t.phase != Phase.Seating && t.phase != Phase.Settled) || t.seats.length < 2) revert InvalidPhase();
    ...
    function forceTimeoutFold(uint256 tableId) external { Table storage t=_table(tableId); require(block.timestamp>t.actionDeadline,"not timed out"); address player=t.seats[t.actingSeat].player; this.actAsTimeout(tableId,player); }
    ...
    function settleShowdown(uint256 tableId, uint8[] calldata values, bytes[] calldata signatures) external {
        ...
        address[] memory winners=_settleSidePots(t,scores);t.phase=Phase.Settled;t.dealer=_nextLive(t,t.dealer);emit HandSettled(tableId,t.handId,winners,t.pot);t.pot=0;
    }
    ...
    function _finishRound(uint256 tableId,Table storage t) private {for(uint256 i;i<t.seats.length;++i){t.seats[i].bet=0;t.seats[i].acted=false;}t.currentBet=0;if(t.phase==Phase.River){t.phase=Phase.Showdown;for(uint256 i;i<t.seats.length;++i)if(t.seats[i].state!=PlayerState.Folded&&t.seats[i].state!=PlayerState.SittingOut){euint8[2] storage h=holeCards[tableId][t.seats[i].player];FHE.allowPublic(h[0]);FHE.allowPublic(h[1]);}return;}t.phase=Phase(uint8(t.phase)+1);uint8 reveal=t.phase==Phase.Flop?3:1;uint256 cursor=_activeSeatCount(t)*2+t.revealedCommunity;for(uint256 i;i<reveal;++i)FHE.allowPublic(decks[tableId][cursor+i]);t.actingSeat=_nextActionable(t,t.dealer);t.actionDeadline=uint64(block.timestamp)+ACTION_WINDOW;}
    function _settleUncontested(uint256 tableId,Table storage t) private {address winner;for(uint256 i;i<t.seats.length;++i)if(t.seats[i].state==PlayerState.Active||t.seats[i].state==PlayerState.AllIn){winner=t.seats[i].player;t.seats[i].stack+=uint96(t.pot);break;}_awardCredit(winner);address[] memory ws=new address[](1);ws[0]=winner;t.phase=Phase.Settled;emit HandSettled(tableId,t.handId,ws,t.pot);t.pot=0;}
    function _settleSidePots(Table storage t,uint256[] memory scores) private returns(address[] memory winners){uint96[] memory levels=new uint96[](t.seats.length);uint256 n;for(uint256 i;i<t.seats.length;++i)if(t.seats[i].committed>0)levels[n++]=t.seats[i].committed;for(uint256 i=1;i<n;++i){uint96 key=levels[i];uint256 j=i;while(j>0&&levels[j-1]>key){levels[j]=levels[j-1];--j;}levels[j]=key;}address[] memory temp=new address[](t.seats.length);bool[] memory credited=new bool[](t.seats.length);uint256 wn;uint96 prev;for(uint256 l;l<n;++l){uint96 level=levels[l];if(level==prev)continue;uint256 layer;for(uint256 i;i<t.seats.length;++i){uint96 capped=t.seats[i].committed<level?t.seats[i].committed:level;if(capped>prev)layer+=capped-prev;}uint256 best;uint256 ties;for(uint256 i;i<t.seats.length;++i)if(t.seats[i].committed>=level&&t.seats[i].state!=PlayerState.Folded&&t.seats[i].state!=PlayerState.SittingOut){if(scores[i]>best){best=scores[i];ties=1;}else if(scores[i]==best)ties++;}if(ties>0)for(uint256 i;i<t.seats.length;++i)if(t.seats[i].committed>=level&&t.seats[i].state!=PlayerState.Folded&&scores[i]==best){t.seats[i].stack+=uint96(layer/ties);if(!credited[i]){credited[i]=true;temp[wn++]=t.seats[i].player;_awardCredit(t.seats[i].player);}}prev=level;}winners=new address[](wn);for(uint256 i;i<wn;++i)winners[i]=temp[i];}
