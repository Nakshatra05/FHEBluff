...
contract FHEBluffReady {
    ...
    function publishCommunity(uint256 tableId, uint8[] calldata values, bytes[] calldata signatures) external {
        Table storage t=_table(tableId);if(t.phase<Phase.Flop||t.phase>Phase.River)revert BadReveal();uint8 expected=_actionableCount(t)==0?5:t.phase==Phase.Flop?3:t.phase==Phase.Turn?4:5;if(values.length==0||values.length!=expected-t.revealedCommunity||signatures.length!=values.length)revert BadReveal();
